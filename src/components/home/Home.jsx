...
export function Home({ onAdd, categoryFilters, setCategoryFilters }) {
  ...
  useEffect(() => {
    if (location.state?.scrollToProductos) {
  ...
  const productosFiltradosPorTexto =
    textFilter.trim() === ''
      ? null
  ...
  const categoriaActiva =
    Object.values(categoryFilters).find(v => v && v !== '') || ''
  ...
  return (
    <main style={{ paddingTop: '80px', minHeight: '60vh' }}>
      ...
      />
      <div id='productos'>
        <ContainerProductos productosData={pagedResults} onAdd={onAdd} />
      </div>
      <Paginación
